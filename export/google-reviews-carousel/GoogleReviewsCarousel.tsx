"use client";
/**
 * GoogleReviewsCarousel — standalone, self-contained review carousel.
 *
 * Requirements on the target site:
 *  - React 18+ (works in Next.js App Router or plain React)
 *  - Tailwind CSS (all styling uses Tailwind utility classes)
 *  - An endpoint that returns { reviews: Review[] } — see api-route.ts
 *
 * Usage:
 *   <GoogleReviewsCarousel />                          // fetches /api/reviews
 *   <GoogleReviewsCarousel endpoint="/api/reviews" />  // custom endpoint
 *   <GoogleReviewsCarousel reviews={myReviews} />      // static data, no fetch
 */
...
interface GoogleReviewsCarouselProps {
  /** Pre-loaded reviews. If provided, no fetching happens. */
  reviews?: Review[];
  /** Endpoint returning { reviews: Review[] }. Default: /api/reviews */
  endpoint?: string;
  lang?: "sk" | "en";
}
    ...
    (async () => {
      try {
        ...
        if (res.ok) {
          ...
          if (!cancelled) setFetchedReviews(data.reviews || []);
        }
      } catch (error) {
    ...
    return (
      <div className="flex justify-center py-8">
        <div className="animate-pulse text-gray-400 text-sm">
          {lang === "en" ? "Loading reviews..." : "Načítavam recenzie..."}
        </div>
      </div>
    );
  }
      ...
      <div className="text-center py-8 text-gray-500">
        <p>
          {lang === "en"
            ...
            : "Momentálne nie sú k dispozícii žiadne recenzie."}
        </p>
      </div>
    );
  }
