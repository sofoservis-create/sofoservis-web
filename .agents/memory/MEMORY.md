...
- [Nimbata DNI vs React DOM](nimbata-dni-dom-conflicts.md) — keep DNI out of dev and lazy-load in prod; it replaces anchors with clones, so preserve a constant-text span and scan all spans.
...
- [Preview CSS after HMR](preview-css-after-hmr.md) — screenshots can briefly mix new markup with stale Tailwind CSS; verify computed styles before changing correct responsive code.
